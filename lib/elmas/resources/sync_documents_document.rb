...
module Elmas
  ...
  class SyncDocumentsDocument
    ...
    def other_attributes
      %i[
        timestamp account account_code account_name amount_fc body category category_description
        contact contact_full_name contract_id contract_number created creator creator_full_name
        currency division document_date document_folder document_folder_code
        document_folder_description document_view_url expiry_date financial_transaction_entry_id
        has_empty_body hid inherit_share item item_code item_description item_material
        item_routing_step language modified modifier modifier_full_name opportunity project
        project_code project_description proposed_entry_status sales_invoice_number
        sales_order_number scan_service_status send_method shop_order_number teams_meeting_id
        type_description
      ]
    end
  end
