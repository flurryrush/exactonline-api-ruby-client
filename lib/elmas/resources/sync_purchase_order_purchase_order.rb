...
module Elmas
  # Endpoint
  # SyncPurchaseOrderPurchaseOrder
  #
  # Good to know
  # Any change in the header or the line of the PurchaseOrders will lead to a new timestamp value in both the header and the line.
  ...
  #  If the line number is 0 it is the purchase order header, if the line number is 1 or higher it is a purchase order line.
  # 
  ...
  # Scope
  # Purchase orders
  class SyncPurchaseOrderPurchaseOrder
    ...
    def mandatory_attributes
      %i[
        item purchase_order_id quantity_in_purchase_units supplier
      ]
    end

    # https://start.exactonline.nl/docs/HlpRestAPIResourcesDetails.aspx?name=SyncPurchaseOrderPurchaseOrders
    def other_attributes
      %i[
        timestamp amount_dc amount_discount amount_discount_excl_vat amount_fc amount_fc_excl_vat
        approval_status approval_status_description approved approver approver_full_name
        cost_center cost_center_code cost_center_description cost_unit cost_unit_code
        cost_unit_description created creator creator_full_name currency custom_field
        delivery_account delivery_account_code delivery_account_name delivery_address
        delivery_contact delivery_contact_person_full_name description discount division document
        document_number document_subject drop_shipment exchange_rate expense expense_description
        incoterm_address incoterm_code incoterm_version invoiced_quantity invoice_status
        is_batch_number_item is_serial_number_item item_barcode item_code item_description
        item_divisable line_number modified modifier modifier_full_name net_price notes order_date
        order_number order_status payment_condition payment_condition_description project
        project_code project_description purchase_agent purchase_agent_full_name quantity rebill
        receipt_date receipt_status received_quantity remarks sales_order sales_order_line
        sales_order_line_number sales_order_number selection_code selection_code_code
        selection_code_description sending_method shipping_method shipping_method_code
        shipping_method_description source supplier_code supplier_contact
        supplier_contact_person_full_name supplier_item_code supplier_item_copy_remarks
        supplier_name unit unit_description unit_price vat_amount vat_code vat_description
        vat_percentage warehouse warehouse_code warehouse_description your_ref
      ]
    end
  end
end
