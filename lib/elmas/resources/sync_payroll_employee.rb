...
module Elmas
  ...
  class SyncPayrollEmployee
    ...
    # https://start.exactonline.nl/docs/HlpRestAPIResourcesDetails.aspx?name=SyncPayrollEmployees
    def other_attributes
      %i[
        timestamp address_line2 address_line3 address_street address_street_number
        address_street_number_suffix birth_date birth_name birth_name_prefix birth_place
        business_email business_fax business_mobile business_phone business_phone_extension city
        code country created creator creator_full_name customer custom_field division email
        employee_hid end_date first_name full_name gender hid initials is_active is_anonymised
        language last_name location_description manager marital_date marital_status middle_name
        mobile modified modifier modifier_full_name name_composition nationality nick_name notes
        partner_name partner_name_prefix person phone phone_extension picture picture_file_name
        picture_thumbnail_url picture_url postcode private_email social_security_number start_date
        state title user user_full_name
      ]
    end
  end
end
