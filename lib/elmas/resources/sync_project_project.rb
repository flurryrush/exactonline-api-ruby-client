...
module Elmas
  ...
  class SyncProjectProject
    ...
    # https://start.exactonline.nl/docs/HlpRestAPIResourcesDetails.aspx?name=SyncProjectProjects
    def other_attributes
      %i[
        timestamp account account_code account_contact account_name allow_additional_invoicing
        allow_member_entry_only block_entry block_invoicing block_planning block_purchasing
        block_rebilling budgeted_amount budgeted_costs budgeted_revenue budget_overrun_hours
        budget_type budget_type_description classification classification_description
        costs_amount_fc created creator creator_full_name customer_p_onumber custom_field division
        division_name enable_work_in_progress end_date fixed_price_item
        fixed_price_item_description has_wbs_lines include_invoice_specification
        include_specification_in_invoice_pdf internal_notes invoice_address invoice_as_quoted
        invoice_description is_wbs_required_for_entry manager manager_fullname markup_percentage
        modified modifier modifier_full_name notes payment_condition prepaid_item
        prepaid_item_description prepaid_type prepaid_type_description sales_time_quantity
        source_quotation start_date time_quantity_to_alert type_description use_billing_milestones
      ]
    end
  end
end
