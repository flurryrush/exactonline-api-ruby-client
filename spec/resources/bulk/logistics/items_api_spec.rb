...
describe Elmas::BulkLogisticsItem do
  ...
  it "returns value for getters" do
    bulk_logistics_item = Elmas::BulkLogisticsItem.new({ "FreeTextField_09" => "345" })
  ...
  context "Applying filters" do
    ...
    it "should apply given filters for find_by" do
      expect(Elmas).to receive(:get).with("bulk/Logistics/Items?$filter=FreeTextField_09 eq '1223'&$filter=ID eq guid'12abcdef-1234-1234-1234-123456abcdef'")
  ...
  context "Applying order" do
    ...
    expect(Elmas).to receive(:get).with("bulk/Logistics/Items?$orderby=FreeTextField_09&$filter=FreeTextField_09 eq '1223'&$filter=ID eq guid'12abcdef-1234-1234-1234-123456abcdef'")
      resource.find_by(filters: [:free_text_field_09, :id], order_by: :free_text_field_09)
    end

    it "should only apply the order_by" do
      expect(Elmas).to receive(:get).with("bulk/Logistics/Items?$orderby=FreeTextField_09")
  ...
  context "Applying select" do
    ...
    expect(Elmas).to receive(:get).with("bulk/Logistics/Items?$select=FreeTextField_09")
    ...
    it "should apply one select" do
    expect(Elmas).to receive(:get).with("bulk/Logistics/Items?$select=FreeTextField_09,ID")
