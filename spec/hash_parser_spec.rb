...
describe Elmas::HashParser do
  ...
  it "parses and returns all results" do
    expect(Elmas::HashParser.new(sample_data).results.length).to eq(2)
  end

  it "parses and returns the first result" do
    expect(Elmas::HashParser.new(sample_data).first_result["FullName"]).to eq("Marthyn Olthof")
    expect(Elmas::HashParser.new(sample_data).first_result["FullName"]).to_not eq("Karel Appel")
  end
end
